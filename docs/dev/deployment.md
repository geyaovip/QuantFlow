...
| Secret   | `PLISIO_API_KEY`           | Plisio 生产会员支付 API key；由 workflow 写入 VPS `.env`   |
| Secret   | `R2_BACKUP_ENDPOINT`       | Cloudflare R2 S3 endpoint；配置后备份 workflow 写入 VPS    |
| Secret   | `R2_BACKUP_BUCKET`         | R2 bucket 名称，例如 `quantflow-backups`                   |
| Secret   | `R2_ACCESS_KEY_ID`         | R2 API token Access Key ID                                 |
| Secret   | `R2_SECRET_ACCESS_KEY`     | R2 API token Secret Access Key                             |

生产备份目录默认为 `/home/ubuntu/apps/quantflow/backups` 与 `/home/ubuntu/apps/quantflow/wal-archive`。首次启用 R2 后执行 `scripts/setup-production-backup-cron.sh` 安装每日全量备份（03:00 UTC）与每小时 WAL 归档 cron；日志写入 `/home/ubuntu/apps/quantflow/logs/`。VPS 需安装 `aws` CLI（`sudo apt-get install -y awscli`）供 R2 同步使用。
...
生产会员支付开启时，workflow 会在 VPS 共享 `.env` 中维护 `ENABLE_PRODUCTION_PAYMENTS=true`、`PUBLIC_WEB_URL=https://quantflow.chat`、`PUBLIC_API_URL=https://api.quantflow.chat` 与 `PLISIO_API_KEY`。不要在仓库、Compose 文件或镜像内硬编码支付密钥。
