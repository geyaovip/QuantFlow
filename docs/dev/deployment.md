...
2. 每日执行完整备份，并在 VPS 本地持续归档 WAL。脚本：`scripts/backup-database.sh`（`pg_dump`）、`scripts/archive-wal.sh`（`pg_switch_wal`）、`scripts/restore-database.sh` 与 `scripts/verify-backup.sh`（恢复演练）。生产 Compose 已启用 `archive_mode`，WAL 写入宿主机 `QUANTFLOW_WAL_ARCHIVE_HOST_DIR`（默认 `/home/ubuntu/apps/quantflow/wal-archive`）。MVP 不做 Cloudflare R2 或其他异地对象存储备份。
3. 本地保留最近 7 天完整备份；季度在隔离环境执行恢复演练。
4. 目标 RPO 与 RTO 以本地备份与 WAL 归档能力为准；不承诺跨机房异地恢复。

## 5. 密钥与可观测性

- Resend、CoinGecko、Turnstile、session、database 密钥只保存在服务器 `shared/.env` 或 CI secrets。
...
生产备份目录默认为 `/home/ubuntu/apps/quantflow/backups` 与 `/home/ubuntu/apps/quantflow/wal-archive`。执行 `scripts/setup-production-backup-cron.sh` 安装每日全量备份（03:00 UTC）与每小时 WAL 归档 cron；日志写入 `/home/ubuntu/apps/quantflow/logs/`。
