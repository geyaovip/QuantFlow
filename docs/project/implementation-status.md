...
状态：持续更新｜最后核对：`v0.6.0-dev.4`（2026-06-26）
...
| 生产部署                  | `partial` | `deploy/`、`scripts/*backup*`、`scripts/archive-wal.sh` | VPS Compose 已上线；备份/恢复/WAL 归档与 R2 同步；生产 PostgreSQL 已启用 `archive_mode` |
...
| 数据库备份到 R2   | `partial` | `scripts/backup-database.sh` 等              | 全量备份、恢复演练、WAL 归档与 R2 同步；季度恢复演练待运维执行            |
