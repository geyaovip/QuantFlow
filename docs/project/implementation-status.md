...
状态：持续更新｜最后核对：`v0.2.0-dev.4`（2026-06-25）
...
| 生产部署                  | `partial` | `deploy/`、`scripts/deploy-production.sh` | VPS Compose 已上线；R2 备份、WAL、Sentry 待落地 |
...
| 应用工作台壳层 | `skeleton`    | `apps/web/app/app/layout.tsx`     | 顶部/底部导航骨架                                   |
| 用户端登录     | `partial`     | `apps/web/app/login`              | 邮箱 OTP + Turnstile UI 已接 API；会话 guard 待接入 |
...
| 管理后台壳层   | `skeleton`    | `apps/admin/app/admin/layout.tsx` | 侧栏与页面框架                                      |
| 管理端登录     | `partial`     | `apps/admin/app/login`            | 邮箱 OTP + Turnstile UI 已接 API；RBAC guard 待接入 |
...
| 邮箱 OTP 认证      | `partial`     | `POST /api/v1/auth/email-otp/*`    | 领域服务、Resend/Turnstile adapter、首版 migration 已有；前端登录流未接 |
...
| 模块              | 状态          | 位置                     | 说明                                   |
| ----------------- | ------------- | ------------------------ | -------------------------------------- |
| Prisma / 数据库   | `partial`     | `apps/api/prisma/`       | 仅 `auth_foundation` migration         |
