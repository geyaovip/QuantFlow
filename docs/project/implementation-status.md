...
状态：持续更新｜最后核对：`v0.6.0-dev.5`（2026-06-26）
...
| 生产部署                  | `partial` | `deploy/`、`scripts/*backup*`、`scripts/setup-production-backup-cron.sh` | VPS Compose 已上线；备份/WAL cron 与脚本已装；R2 待 API token 配置 |
...
| 数据库备份到 R2   | `partial` | `scripts/backup-database.sh` 等              | 全量/WAL 本地归档与 cron 已装；R2 endpoint/bucket 已写入 VPS，API key 待配置 |
