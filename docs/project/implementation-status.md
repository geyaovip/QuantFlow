...
状态：持续更新｜最后核对：`v0.4.0-dev.2`（2026-06-26）
...
| 策略广场       | `partial`     | `/app/strategies`                 | 已接策略 API；支持风险/类型/币种/排序筛选；列表按会员档位过滤                 |
...
| 模拟盘         | `partial`     | `/app/paper-trading`              | 创建/列表/详情、暂停/恢复/结束/复制/删除；历史最大回撤与行情快照校验          |
...
| 管理端模拟盘   | `partial`     | `/admin/paper-accounts`           | 已接列表、暂停/恢复/标记异常；写审计日志                                      |
...
| 模拟盘             | `partial`     | `GET/POST/DELETE /api/v1/paper-accounts*`、`/admin/paper-accounts*`                           | 生命周期（结束/复制/软删除）、分页子资源、历史最大回撤、`market_price_snapshots` 校验                               |
...
| 模块              | 状态          | 位置                     | 说明                                                         |
| ----------------- | ------------- | ------------------------ | ------------------------------------------------------------ |
| Prisma / 数据库   | `partial`     | `apps/api/prisma/`       | 认证、策略/信号、会员、模拟盘、行情快照表 migration 已有     |
| 行情采集          | `partial`     | `apps/worker`            | Worker 仍仅 heartbeat；行情快照由 migration 种子与执行时写入 |
