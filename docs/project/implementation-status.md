...
状态：持续更新｜最后核对：`v0.3.0-dev.3`（2026-06-26）
...
| 策略广场       | `partial`     | `/app/strategies`                 | 已接策略 API 与服务端风险筛选、分页                 |
| 策略详情       | `partial`     | `/app/strategies/[id]`            | 已接详情 API 与订阅/取消；模拟盘创建仍待接入        |
| 信号中心       | `partial`     | `/app/signals`                    | 已接信号列表/详情、方向筛选与分页；收藏、提醒待接入 |
| 我的策略       | `partial`     | `/app/my-strategies`              | 已展示当前订阅策略；配额来自后端                    |
| 模拟盘         | `skeleton`    | `/app/paper-trading`              | 静态空态/示例                                       |
| 个人中心       | `partial`     | `/app/profile`                    | 已展示会话与 Free 订阅配额；通知与安全记录待接入    |
...
| 策略               | `partial`     | `GET/POST /api/v1/strategies*`、`/api/v1/admin/strategies*` | 用户端列表/详情/订阅与管理端最小审核流已接；完整 RBAC 待接                                  |
| 信号               | `partial`     | `GET /api/v1/signals*`                                      | 按订阅可见的 active 信号列表与详情；支持 `direction` 筛选与分页；收藏、提醒和加入模拟盘待接 |
...
| 模块              | 状态          | 位置                     | 说明                                                         |
| ----------------- | ------------- | ------------------------ | ------------------------------------------------------------ |
| Prisma / 数据库   | `partial`     | `apps/api/prisma/`       | 认证、管理员预授权、策略/信号、订阅和策略审计 migration 已有 |
