...
状态：持续更新｜最后核对：`v0.3.0-dev.5`（2026-06-26）
...
| 策略详情       | `partial`     | `/app/strategies/[id]`            | 已接详情 API、多周期指标、历史信号与订阅/取消；不足档位展示升级引导    |
| 信号中心       | `partial`     | `/app/signals`                    | 已接信号列表/详情、方向/状态筛选与分页；历史天数与 Free 延迟由后端裁剪 |
| 我的策略       | `partial`     | `/app/my-strategies`              | 已展示当前订阅策略；配额来自会员权益；空态和概览已优化                 |
| 模拟盘         | `skeleton`    | `/app/paper-trading`              | 正式功能占位页；创建、持仓、成交和风控记录待接入                       |
