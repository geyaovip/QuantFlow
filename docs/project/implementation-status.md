...
状态：持续更新｜最后核对：`v0.6.0-dev.3`（2026-06-26）
...
| 生产部署                  | `partial` | `deploy/`、`scripts/*backup*`、`scripts/archive-wal.sh` | VPS Compose 已上线；备份/恢复/WAL 轮转脚本；连续 WAL 归档待生产配置 |
...
| 策略广场       | `partial` | `/app/strategies`                            | 已接策略 API；支持风险/类型/币种/模拟盘筛选与排序                             |
| 策略详情       | `partial` | `/app/strategies/[id]`                       | 已接详情 API、多周期指标、历史信号与订阅/取消；已订阅用户可直接创建模拟盘     |
| 信号中心       | `partial` | `/app/signals`                               | 已接信号列表/详情、方向/状态筛选与分页；active 信号可创建模拟盘并执行模拟成交 |
...
| 会员订阅       | `partial` | `/app/membership`                            | 计划列表、Plisio 支付跳转和支付确认；回调成功后自动开通                       |
...
| 管理端其他页   | `partial` | `/admin/memberships`、`/admin/announcements` | 会员订阅列表/取消与系统公告创建/发布已接 API                                  |
...
| 会员 / 权益        | `partial` | `GET/POST /api/v1/membership/*`                                                               | 计划、订阅、权益查询、Plisio checkout/callback 与测试 mock-checkout；生产支付由 env 开关控制 |
...
| 管理端治理         | `partial` | `GET /admin/dashboard`、`/admin/users*`、`/admin/subscriptions*`、`/admin/announcements*`     | 看板、用户状态、人工开通会员、订阅取消、系统公告发布                                         |

## 数据与后台任务

| 模块              | 状态      | 位置                                         | 说明                                                                     |
| ----------------- | --------- | -------------------------------------------- | ------------------------------------------------------------------------ |
| Prisma / 数据库   | `partial` | `apps/api/prisma/`                           | 认证、策略/信号、会员、模拟盘、RBAC、通知、风险事件、公告 migration 已有 |
...
| E2E 冒烟测试      | `partial` | `packages/e2e`、`.github/workflows/e2e.yml`  | API/Web 登录 journey、模拟盘创建、营销页性能；CI 可手动触发全栈 E2E      |
| 数据库备份到 R2   | `partial` | `scripts/backup-database.sh` 等              | 全量备份、恢复演练、WAL 轮转；连续 archive_mode 待生产 PostgreSQL 配置   |
| 可观测性          | `partial` | `SENTRY_DSN`、`NEXT_PUBLIC_SENTRY_DSN`       | API + Web/Admin 客户端 Sentry 可选接入                                   |
