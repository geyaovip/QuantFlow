...
状态：持续更新｜最后核对：`v0.7.0-dev.2`（2026-06-29）
...
| CI 门禁                   | `done`    | `.github/workflows/ci.yml`、`.github/workflows/e2e.yml`                  | `pnpm check` + `pnpm build` + 主 CI 全栈 E2E                      |
| 生产部署                  | `partial` | `deploy/`、`scripts/*backup*`、`scripts/setup-production-backup-cron.sh` | VPS Compose 已上线；生产 migration 已跑；本地备份与 WAL cron 已装 |
...
| 策略广场       | `partial` | `/app/strategies`                            | 已接策略 API；支持风险/类型/币种/免费/模拟盘/收益周期/最大回撤筛选与多维度排序                       |
| 策略详情       | `partial` | `/app/strategies/[id]`                       | 已接详情 API、多周期指标、收益/回撤曲线、历史信号与订阅/取消；已订阅用户可直接创建模拟盘             |
| 信号中心       | `partial` | `/app/signals`                               | 已接信号列表/详情；支持方向/状态/是否已用于模拟筛选；有效信号可进入模拟盘创建路径                    |
...
| 会员订阅       | `partial` | `/app/membership`                            | Free/Plus/Pro 计划列表、Plisio 美元支付、邀请码兑换与 mock 开通；支付确认弹窗与权益概览已优化        |
| 通知中心       | `partial` | `/app/notifications`                         | 站内通知列表、标记已读与偏好设置页                                                                   |
| 管理后台壳层   | `partial` | `apps/admin/app/admin/layout.tsx`            | 侧栏分组导航、管理员信息与退出入口；proxy 已强制管理员登录                                           |
...
| 管理端用户治理 | `partial` | `/admin/users`、`/admin/users/[userId]`      | 用户列表、状态变更、人工开通会员与用户详情运营视图                                                   |
...
| 管理端其他页   | `partial` | `/admin/memberships`、`/admin/announcements` | 会员订阅列表/取消、Plisio 支付只读核查、邀请码创建/停用与系统公告创建/发布已接 API；创建入口默认收起 |
...
| 会员 / 权益        | `partial` | `GET/POST /api/v1/membership/*`                                                                                                                             | 计划、订阅、权益、邀请码兑换、Plisio checkout/callback 与非生产测试 mock-checkout           |
...
| 管理端治理         | `partial` | `GET /admin/dashboard`、`/admin/users*`、`/admin/subscriptions*`、`/admin/membership-payments*`、`/admin/membership-invite-codes*`、`/admin/announcements*` | 看板、用户状态、人工开通会员、支付核查、邀请码管理、订阅取消、系统公告发布                  |
...
| E2E 冒烟测试      | `done`    | `packages/e2e`、`.github/workflows/e2e.yml`  | API/Web 登录 journey、模拟盘创建、策略筛选、用户详情、管理端治理与策略页性能已纳入主 CI        |
