...
- 摘要：收束 v0.3 权限与体验：服务端强制执行 `requiredTier`、历史信号窗口与 Free 15 分钟延迟；策略广场补币种筛选；官网定价接会员计划 API；不足档位展示升级引导；并优化官网与用户端页面层级、空态和导航。
- MVP 边界影响：无；仍为模拟开通、无真实支付、无模拟盘/通知/完整 RBAC。
- API / 数据 / 权限 / 风控影响：策略列表按用户档位过滤；订阅与详情校验 `requiredTier`；信号按 `history_days` 与 Free 延迟裁剪；新增 Pro 示例策略 seed migration `202606260003_v03_pro_strategy_seed`。
- 迁移与兼容：新增 migration `202606260003_v03_pro_strategy_seed`。
- 验证：`pnpm check`。
- 监控与回滚：如档位过滤或页面布局异常，可回滚 release 镜像；seed 数据可保留。
...
- 摘要：优化官网产品预览区块，移除侧栏小标题造成的视觉错位；新增 `/app/strategies/[strategyId]` 策略详情静态骨架，策略卡片可进入详情页查看当前信号、收益/回撤、胜率/样本、适合与不适合行情及标准风险披露。
- MVP 边界影响：无；未增加交易所连接、真实下单、半自动/自动交易或在线支付入口。模拟盘创建入口保持禁用，等待后续业务切片接入。
- API / 数据 / 权限 / 风控影响：无新增 API 或数据库迁移；策略详情仍使用前端静态示例数据，不代表后端策略模块已实现。
- 迁移与兼容：无。
- 验证：待本切片完成时运行 `pnpm --filter @quantflow/web typecheck`、`pnpm check`。
- 监控与回滚：如官网布局或详情路由出现问题，可回滚前端页面和样式变更；不影响认证、API 或生产数据。
