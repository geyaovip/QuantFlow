...
- 类型：Changed / Frontend / Design / Docs
- 摘要：优化官网板块布局、卡片层级和风险表达；统一用户端策略、信号、详情、我的策略、模拟盘、会员和个人中心的页面概览、空态、禁用操作和风险提示；桌面侧边栏补充会员权益和个人中心入口。
- MVP 边界影响：无；未增加交易所连接、真实下单、半自动/自动交易、生产支付或模拟盘创建能力。
- API / 数据 / 权限 / 风控影响：无新增 API、数据库迁移或权限变化；仅调整现有页面结构、样式和交互状态。
- 迁移与兼容：无。
- 验证：待本切片完成时运行 `pnpm --filter @quantflow/web typecheck`、`pnpm check`、`pnpm build`。
- 监控与回滚：如页面布局或交互状态异常，可回滚前端页面与样式变更；不影响生产数据。
